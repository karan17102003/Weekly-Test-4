...
const [age, setAge] = useState(0);
  function handleOnClick(employee) {
    setSelectedEmployees([...selectedEmployees, {
      "id":employees[employee-1].id,"first_name":employees[employee-1].first_name,"last_name":employees[employee-1].first_name,"email":employees[employee-1].last_name,"gender":employees[employee-1].gender,"age":employees[employee-1].age
    }]);
    setAge((age+employees[employee-1].age))
    
  }
  

  function handleRemove(employee) {
    const newList = selectedEmployees.filter((item) => item.id!== employee);
  ...
  return (
    <div className='maincontainer'>
      <div className='liftDiv'>
        <h2>Employees</h2>
        {
      employees.map((item) => <div className='employe-list' id={item}>
      ...
      {selectedEmployees.map((employee) => (
          <div key={employee.id} className='employe-list'>
      ...
      </div><h3 className='avgAge'>Avg Age :{Math.floor(age/selectedEmployees.length)}</h3>
    </div>
  )
}
